...
<html>
<head>
  <title>Receptionist Update</title>
...
//---------------------------contact---------------------------
  ...
  else
  {
    ...
    {
      $NID_No_Error = "NID Number is less than 14";
    }
    else if (!preg_match('/^[0-9]{14}+$/', $NID_No1)) 
    {  
      $NID_No_Error = "Invalid MID number"; 
    }
    else
    {
      $NID_No = $_POST["t_no"];
...
if(isset(($_POST['Update'])))
{
  ...
  if(($name != "")and ($nameErr == "") and($dayErr == "")and($monthErr == "")and($yearErr == "")and($genderErr == "")and($B_GErr == "")and($Present_Add_Error == "")and($Permanent_Add_Error == "")and($Mail_Error == "")and($Mobile_No_Error == "")and($NID_No_Error == ""))
  {
    ...
    $sql = "UPDATE User SET Name='".$name."',DOB='".$DOB."', Gender='".$GENDER."', B_G='".$Blood_G."', Age='".$age."', Pre_Add='".$Present_Add."', Per_Add='".$Permanent_Add."', Email='".$Mail."', Mobile='".$Mobile_No."', NID='".$NID_No."', Photo='".$uploaded_image."'  WHERE U_ID='".$id."'";
